...
namespace bloquesModelo\informesFinalesSinJurados;
...
class Sql extends \Sql {
	...
	function getCadenaSql($tipo, $variable = '') {
		...
		switch ($tipo) {
			...
			case 'buscarInformes' :
				$cadenaSql = 'SELECT ';
				$cadenaSql .= 'i.info_fcrea as FECHA, ';
				$cadenaSql .= 'i.info_info as INFORME, ';
				$cadenaSql .= 'm.moda_nombre as MODALIDAD, ';
				$cadenaSql .= 'i.info_titu as TITULO, ';
				$cadenaSql .= 'i.info_einfo as ESTADO ';
				$cadenaSql .= 'FROM ';
				$cadenaSql .= 'trabajosdegrado.inf_tinfo i, ';
				$cadenaSql .= 'trabajosdegrado.ge_tmoda m ';
				$cadenaSql .= 'WHERE ';
				$cadenaSql .= "i.info_einfo='RADICADO' ";
